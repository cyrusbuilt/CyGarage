...
void ConsoleClass::onActivateDoor1(void (*activateDoor1Handler)()) {
    this->activateDoor1Handler = activateDoor1Handler;
}

#ifdef CG_MODEL_2
void ConsoleClass::onActivateDoor2(void (*activateDoor2Handler)()) {
    this->activateDoor2Handler = activateDoor2Handler;
}
#endif
...
void ConsoleClass::displayMenu() {
    ...
    Serial.println(F("=                            ="));
    #ifdef CG_MODEL_2
        Serial.println(F("= a: Activate Door 1         ="));
...
void ConsoleClass::checkCommand() {
    ...
    switch (incomingByte) {
        case 'a':
            if (this->activateDoor1Handler != NULL) {
                this->activateDoor1Handler();
            }
            break;
        #ifdef CG_MODEL_2
        case 'b':
            if (this->activateDoor2Handler != NULL) {
                this->activateDoor2Handler();
            }
            break;
        #endif
        case 'r':
